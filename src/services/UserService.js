...
const createUser = (newUser) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const createdUser = await User.create({name, email, password: hash, confirmPassword})
            if(createdUser) {
                resolve({
                    status: 'OK',
                    message: 'SUCCESS',
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            if(filter) {
                ...
                const allObjectFilter = await User.find({ [label]: { '$regex': filter[1]}})
                resolve({
                    status: 'OK',
                    message: 'Success',
...
const getFollowingUser = (userId) => {
    return new Promise(async (resolve, reject) => {
            try {
                ...
                const followings = await Promise.all(user1.followings.map((following) => {
...
const getNotFollowingUser = (userId) => {
    return new Promise(async (resolve, reject) => {
            try {
                ...
                let NotFollowingList = await User.find({
                }, {"_id": 1 , "name" : 1});
                
                followingsList.map((followingItem) => {
                    NotFollowingList = NotFollowingList.filter((NFL) => (NFL._id != `${followingItem._id}`))
                })
                NotFollowingList = NotFollowingList.filter((NFL) => (NFL._id != `${userId}`))
                if(user1 === null) {
                    resolve({ 
                        status: 'OK',
                        message: 'User is not defined'
