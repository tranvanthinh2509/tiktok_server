...
const createVideo = (newVideo) => {
    return new Promise(async (resolve, reject) => {
        const { description, video,tag, imageBg, liked, comment, saved, shared, userId } = newVideo
        try {
            
    
            const newVideo = await Video.create({description, video,tag, imageBg, liked, comment, saved, shared, userId})
    ...
    return new Promise(async (resolve, reject) => {
        try {
            const VideoID = await Video.findOne({
                _id: id
            }).populate({path: 'userId', select: ['name', 'nickName', 'avatar','followings','followers','creatAt']})
